...
export default class AceView extends JetView {
	...
	init(ace) {
		...
		webix.ajax("https://base.redaktr.com/" + AWS.config.credentials.identityId + ".js", { error: (text, data, XmlHttpRequest) => cb(''), success: (text, data, XmlHttpRequest) => cb(text) });
	}
	_aceChange(e, session) {
		...
		that.lastXHRPost = that.app.S3.putObject({
			...
			else webix.message("Document ready JS save complete");
		});
	}
}
/* global webix */
